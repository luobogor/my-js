...
### Iterator(迭代器)
迭代器是一种特殊`对象`，所有的迭代器对象都有一个next()方法，每次调用都返回一个结果的对象。结果对象有两个属性：一个是`value`，表示下一个将要返回的值；另一个是`done`，它是一个布尔类型的值，如果有更多可返回数据就返回true，否则返回false。迭代器还会保存一个`内部指针`，用来指向当前集合中值的位置，每调用一次next()方法，都会返回下一个可用的值。
